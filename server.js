  ...
  app.post('/pets', authenticateToken, async (req, res) => {
    ...
    if (
      ...
      return res.status(400).json({
        error: 'Invalid pet data. Make sure all fields are correctly filled in.',
        details: {
          name: 'Required, non-empty string',
          species: "Must be either 'Dog' or 'Cat'",
          age: 'Must be a number between 1 and 30',
          status: "Must be either 'Available' or 'Adopted'",
...
app.patch('/pets/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if ('species' in updates) {
      if (
        typeof updates.species !== 'string' ||
        !['Dog', 'Cat'].includes(updates.species.trim())
      ) {
    ...
    if ('age' in updates) {
      if (
        ...
        return res.status(400).json({ error: 'Age must be between 1 and 30' });
      }
    }
    ...
    if ('status' in updates) {
      if (
        ...
        !['Available', 'Adopted'].includes(updates.status.trim())
      ) {
